...
{
    public class CraneCosmeticIndicator : ResponsiveObjectView<CraneCosmeticIndicator.ViewData, CraneCosmeticIndicator.ResponseData>, IInputConsumer
    {
        public class UpdateView : ResponsiveViewSystemBase<ViewData, ResponseData>, IModSystem
        {
            ...
            {
                base.Initialise();
                _indicators = GetEntityQuery(typeof(CLinkedView), typeof(CCraneEditorInfo));
            }
            ...
            protected override void OnUpdate()
            {
                using (NativeArray<Entity> indicators = _indicators.ToEntityArray(Allocator.Temp))
                {
                    foreach (Entity indicator in indicators)
                    {
                        ...
                        if (!Require(indicator, out CCraneEditorInfo craneEditorInfo)) continue;
